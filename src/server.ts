...
server.registerTool('propose_algorithm', {
  ...
  const cand: string[] = [];
  if (/shortest|кратч|путь/.test(p)) cand.push('dijkstra');
  if (/подстр|substring|pattern|шаблон/.test(p)) cand.push('kmp');
  if (/компонент|соед|cluster|объедин/.test(p)) cand.push('dsu');
...
server.registerTool('verify_complexity', {
  ...
  const gen = (n: number) => [Array.from({ length: n }, (_, i) => n - i)];
...
server.registerTool('cve_check', {
  ...
  const fixes = res.vulns?.flatMap((v: any) => v.affected?.flatMap((a: any) => a.ranges?.flatMap((r: any) => (r.events || []).map((e: any) => e.fixed).filter(Boolean)))) || [];
...
server.registerTool('suggest_safe_version', {
  ...
  const res = await suggestSafeVersion(ecosystem as any, name);
